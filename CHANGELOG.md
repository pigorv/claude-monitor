...
- The session detail API now exposes a `token_budget` breakdown (billed tokens, total cost, parent vs. sub-agent split, per-token-type usage, and peak context) for each session.

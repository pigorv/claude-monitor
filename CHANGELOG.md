...
- Support for the **Fable 5** model (`claude-fable-5`). Fable sessions are now measured against their 1M-token context window (correct context-pressure % and chart threshold lines), show a cost estimate ($10/$50 per MTok) in the session list and `/api/stats` totals, render a styled **Fable** pill with a 1M badge, and can be isolated with a Fable model filter.

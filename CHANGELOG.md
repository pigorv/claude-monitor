...
- Session Detail timeline: `AskUserQuestion` tool calls now render as a review-mode card. **Collapsed row** shows the badge, an `N questions` chip, the first question (truncated), and a status pill (green `Answered`, amber `Partial`, gray `Skipped`, or green `Free-form`). **Expanded view** has an accent left rail and renders each question as its own block with a `Q1/Q2/…` counter (omitted for single-question cards), per-question status pill, and a hover-revealed Copy button that puts a formatted `Q: … / Options: … / A: …` block on the clipboard. Answered questions show all options in a two-column grid: the picked one carries the accent rail + tinted background + check icon, alternatives mute to gray. Skipped questions render `User declined to answer · agent will proceed with the default`. Free-form (typed) answers get a tinted accent block with `CUSTOM ANSWER` label plus an `Original options offered: …` footnote, and multi-select free-form mixes both highlighted options and the custom text. Rejection-with-clarification (user declined but the SDK preserved partial picks) now shows as Partial rather than a flat error, so the captured answers stay visible. Falls back gracefully when the body is a true tool error.

...
- A schema-drift canary (`test/fixtures/schema-drift-canary.test.ts`) scans the fixture corpus against a manifest of known JSONL fields (`src/ingestion/jsonl-schema-manifest.ts`) and surfaces any unrecognized message/usage/content field — a warning on PR runs, and a blocking `npm run test:schema-drift` job in nightly CI — so Claude Code transcript-format drift is caught instead of silently dropped.

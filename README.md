...
**Clone a Session** — Re-root any session in another project directory and pick the conversation back up there. The Clone button in the Session Detail header opens a modal prefilled with the session's recorded path; point it somewhere else and claude-monitor writes the transcript — sub-agent files included — under a fresh session id with its working directory repointed. You get the new `claude --resume <id>` to copy, a one-click Open in Terminal, and a link straight to the clone's own detail page. Expired sessions can't be cloned, since cloning needs the raw transcript that retention cleanup already removed.

<img src="https://raw.githubusercontent.com/pigorv/claude-monitor/main/docs/images/clone.gif" alt="Clone modal on a session detail page: the target directory is retyped to a different project folder, then the success view shows the cloned session's claude --resume command, an Open in Terminal button, and a link to the clone" width="700" />
...
<img src="https://raw.githubusercontent.com/pigorv/claude-monitor/main/docs/images/export.gif" alt="Export modal on a session detail page offering two bundle formats — Sanitized, badged Recommended, and Raw, badged Unsanitized — each with an inline explanation of what it contains" width="700" />

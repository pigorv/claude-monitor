...
describe('parseTranscriptWithTitle (title extraction)', () => {
  ...
  it('returns customTitle from the fixture file', async () => {
    const { title } = await parseTranscriptWithTitle(FIXTURE_PATH);
  ...
  it('returns null when no custom-title line exists', async () => {
    ...
    const { title } = await parseTranscriptWithTitle(filePath);
